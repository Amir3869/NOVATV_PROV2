...
/**
 * Lecture native Android par ExoPlayer, sous la WebView.
 *
 * Chrome Android ne décode pas toujours les codecs IPTV (notamment AC-3) :
 * ExoPlayer prend en charge les VOD et les directs HTTP. Le chrome HTML
 * reste au-dessus (WebView transparente), tandis que la surface native
 * continue d'être peinte en PiP.
 */
@CapacitorPlugin(name = "NativeVodPlayer")
public class NativeVodPlayerPlugin extends Plugin {
    ...
    @Nullable private PlayerView playerView;
    @Nullable private ViewGroup host;
    @Nullable private static NativeVodPlayerPlugin activeInstance;
    private int webViewColor = Color.BLACK;
    private final Handler main = new Handler(Looper.getMainLooper());
    @Nullable private Runnable tick;
    private float lastVolume = 1f;
    ...
    /** Appelé par MainActivity sur Android 7–11, où l'auto-PiP n'existe pas. */
    public static void enterFromSystem() {
        if (activeInstance != null) {
            activeInstance.enterPictureInPictureInternal();
        }
    }
    ...
    public void play(PluginCall call) {
        ...
        getActivity().runOnUiThread(() -> {
            ...
            exo.play();
            player = exo;
            mediaSession = new MediaSession.Builder(getContext(), exo)
                    .setId("NovaTV")
                    .build();
    ...
    public void enterPictureInPicture(PluginCall call) {
        ...
        getActivity().runOnUiThread(this::enterPictureInPictureInternal);
        call.resolve();
    }
    ...
    protected void handleOnPause() {
        super.handleOnPause();
        // En PiP, ExoPlayer doit continuer. En pause classique, on
        // conserve le comportement existant pour économiser la batterie.
    ...
    private PictureInPictureParams pictureInPictureParams(boolean autoEnter) {
